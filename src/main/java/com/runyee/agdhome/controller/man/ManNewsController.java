...
@UrlSkip(value = "man_news",skip = Skip.MAN)
public class ManNewsController {
    @Autowired
    ...
    public AppJson source_list(HttpServletRequest request){
        AppJson app = new AppJson();
        String search = request.getParameter("search");
        String appcode = service.man_source_list(search,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    public AppJson list(ManNewsPageBean pageBean){
        AppJson app = new AppJson();
        String appcode = service.man_news_list(pageBean,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    public AppJson content(HttpServletRequest request){
        AppJson app = new AppJson();
        String id = request.getParameter("id");
        String appcode = service.man_content(id,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    public AppJson add_edit(NewsForm form){
        AppJson app = new AppJson();
