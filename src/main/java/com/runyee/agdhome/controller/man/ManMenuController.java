...
@UrlSkip(value = "man_menu",skip = Skip.MAN)
public class ManMenuController {
    ...
    public AppJson shows(){
        AppJson app = new AppJson();
        String appcode = service.shows(app);
    ...
    public AppJson menus(){
        AppJson app = new AppJson();
        String appcode = service.menus(app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    public AppJson right(HttpServletRequest request){
        AppJson app = new AppJson();
        String id = request.getParameter("id");
        String opt = request.getParameter("opt");//1.关联
        String appcode = service.right(id,opt,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    public AppJson view(HttpServletRequest request){
        AppJson app = new AppJson();
        String id = request.getParameter("id");
        String opt = request.getParameter("opt");//1.可见
        String appcode = service.view(id,opt,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    public AppJson issue(MenuForm formBean){
        AppJson app = new AppJson();
    ...
    public AppJson sort(HttpServletRequest request){
        ...
        String appcode = service.sort(id,opt,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
