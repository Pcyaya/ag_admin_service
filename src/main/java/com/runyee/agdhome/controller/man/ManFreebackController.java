 ...
 */
@RestController
@RequestMapping("/man/freeback")
@UrlSkip(value = "man_freeback",skip = Skip.MAN)
public class ManFreebackController {
    @Autowired
    private AppFreebackService service;
    @Autowired
    private FreebackService serviceMan;
    ...
    public AppJson list(ManAppFreeBackPageBean pageBean){
        AppJson app = new AppJson();
        String appcode = serviceMan.manlist(pageBean,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
     ...
     * 投诉建议 删除
     * */
    @RequestMapping(value="/mandel", method = RequestMethod.POST)
    public AppJson del(HttpServletRequest request){
        AppJson app = new AppJson();
        String id = request.getParameter("id");
        String appcode = serviceMan.mandel(id,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    @RequestMapping(value="/maninfo", method = RequestMethod.POST)
    public AppJson maninfo(HttpServletRequest request){
        AppJson app = new AppJson();
        String id = request.getParameter("id");
        String appcode = serviceMan.maninfo(id,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
    ...
    @RequestMapping(value="/manhandle", method = RequestMethod.POST)
    public AppJson manhandle(FreebackHandleForm form){
        AppJson app = new AppJson();
        String appcode = serviceMan.manhandle(form,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
