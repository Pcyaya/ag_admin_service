...
import com.runyee.agdhome.service.SosService;
import com.runyee.agdhome.service.ValidService;
 ...
 */
@RestController
@RequestMapping("/man/valid")
@UrlSkip(value = "man_valid",skip = Skip.MAN)
public class ManValidController {
    @Autowired
    ...
    public AppJson list(ManValidPageBean pageBean){
        AppJson app = new AppJson();
    ...
    public AppJson info(HttpServletRequest request){
        AppJson app = new AppJson();
        String id = request.getParameter("id");
        String view = request.getParameter("view");//1.预览
    ...
    public AppJson inform(GuideValidHandleForm form){
        AppJson app = new AppJson();
        String appcode = service.audit(form,app);
        if(APPCode.codeMap.containsKey(appcode)){
            //原编码返回
            app.setSuccess(appcode);
            if(!APPCode.SUCCESS.equals(appcode)){
